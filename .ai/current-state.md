...
- deterministic read-only project dashboard snapshot and static-site builder derived from experiment/knowledge registries and the configured universe
- optional public GitHub live enrichment for operational presentation only, with canonical research status retained offline and unchanged
- dashboard CI workflow that builds review artifacts on pull requests and deploys only from non-PR main/scheduled/manual runs
